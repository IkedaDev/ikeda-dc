// src/application/use-cases/GetWelcomeMessage.ts
...
export class GetWelcomeMessageUseCase {
  private templates: WelcomeTemplate[] = [
  ...
  async execute(username: string): Promise<WelcomeData> {
    const randomIndex = Math.floor(Math.random() * this.templates.length);
    const selected = this.templates[randomIndex];
